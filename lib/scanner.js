...
async function runScanner(options = {}) {
  ...
  const portfolio = buildPortfolioSnapshot(portfolioRows, latestMarketDate);
  const portfolioContext = scannerContextFromPortfolio(portfolio);
  const execution = executionFilter(candidates, portfolioContext);
