...
// Las reglas son las mismas de siempre; lo que se ha movido fuera es la
// matematica (lib/indicators.js), la descarga (lib/yahoo.js) y el universo
// (lib/universe.js), para que la estrategia de insiders reutilice todo eso.
...
async function runScanner(options = {}) {
  ...
  return {
    ...
    // Solo para el tracker: permite reutilizar los precios ya descargados sin
    // volver a pedirlos. La API nunca lo serializa (pesa megas).
