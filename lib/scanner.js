...
async function fetchChart(symbol, timeoutMs = 15000) {
  ...
  if (Number.isFinite(regularPrice) && Number.isFinite(regularTime) && rows.length) {
    ...
    if (last.date === regularDate) {
      ...
      last.high = Number.isFinite(last.high) ? Math.max(last.high, regularPrice) : regularPrice;
      last.low = Number.isFinite(last.low) ? Math.min(last.low, regularPrice) : regularPrice;
...
function addTradePlan(row) {
  ...
  const stopLossPct = isMomentum ? clip(1.65 * dailyVol, 0.045, 0.075) : clip(2.0 * dailyVol, 0.05, 0.11);
  const takeProfitPct = isMomentum ? clip(1.75 * stopLossPct, 0.07, 0.13) : clip(1.9 * stopLossPct, 0.08, 0.20);
  const entryZoneLow = row.price * (isMomentum ? 0.992 : 0.994);
  const entryZoneHigh = row.price * (isMomentum ? 1.000 : 1.002);
...
function executionFilter(rows, portfolioContext = {}) {
  ...
  const evaluated = rows.map((row) => {
    ...
    const rrMin = isMomentum ? MIN_MOMENTUM_EXECUTION_RR : MIN_CORE_EXECUTION_RR;
    ...
    const relativeStrengthOk = isMomentum
      ? Number.isFinite(rs1m) && rs1m >= MOMENTUM_MIN_RS_1M_VS_SPY && Number.isFinite(rs3m) && rs3m >= MOMENTUM_MIN_RS_3M_VS_SPY
      : !Number.isFinite(rs3m) || rs3m >= -0.025;
    const volumeConfirmationOk = row.volume_ratio_20_60 >= (isMomentum ? MOMENTUM_MIN_VOLUME : CORE_MIN_VOLUME);
    const momentumQualityOk = !isMomentum || (row.opt_score >= MOMENTUM_MIN_OPT_SCORE && row.rank_today <= MOMENTUM_MAX_RANK);
    const sector = row.gics_sector || "SIN_SECTOR";
    const openSectorCount = Number(openSectorCounts.get(sector) || 0);
    const openMomentumCount = Number(openStrategyCounts.get("BREAKOUT_CONTINUATION") || 0);
