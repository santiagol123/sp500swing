...
function formatSecAcceptanceDateTime(value) {
  ...
  return `${match[1]}-${match[2]}-${match[3]} ${match[4]}:${match[5]}:${match[6]} ET`;
}
...
async function fetchRecentForm4Filings(cik, sinceDate) {
  ...
  for (let i = 0; i < recent.form.length; i += 1) {
    ...
    out.push({
      ...
      filingAcceptedAt: formatSecAcceptanceDateTime(recent.acceptanceDateTime?.[i]),
...
async function fetchForm4(cik, filing) {
  ...
  return {
    ...
    filing_datetime: filing.filingAcceptedAt || filing.acceptanceDateTime || null,
    filing_acceptance_datetime: filing.acceptanceDateTime || null,
  };
}
...
function extractInsiderBuys(form4, { includeTenPercentOwners = false } = {}) {
  ...
  return insiders.map((owner) => ({
    ...
    filing_datetime: form4.filing_datetime || null,
    filing_acceptance_datetime: form4.filing_acceptance_datetime || null,
