...
const MIN_STANDALONE_SENIOR_VALUE_USD = Number(process.env.INSIDER_TOTAL_MIN_STANDALONE_SENIOR_VALUE_USD || 500000);
...
function compactInsiders(rows, defaultSource = "dataroma_flow") {
  ...
  for (const row of rows) {
    ...
    const current = byOwner.get(key) || {
      ...
      filing_datetime: row.filing_datetime || null,
    ...
    if (row.filing_datetime && (!current.filing_datetime || row.filing_datetime > current.filing_datetime)) {
      current.filing_datetime = row.filing_datetime;
    }
...
function technicalMeta(feature) {
  return {
    price: feature.price,
    prev_close: feature.prev_close,
    entry_gap_1d: feature.ret_1d,
    ret_1w: feature.ret_1w,
    ret_1m: feature.ret_1m,
    dist_sma20: feature.dist_sma20,
    dist_sma50: feature.dist_sma50,
    above_sma20: Boolean(feature.above_sma20),
    above_sma50: Boolean(feature.above_sma50),
    volume_ratio_20_60: feature.volume_ratio_20_60,
    dollar_volume_20d: feature.dollar_volume_20d,
    rsi14: feature.rsi14,
  ...
  for (const [ticker, rows] of buysByTicker) {
    ...
    const summaries = [...byOwner.values()]
      ...
      .map((summary) => {
        ...
        const filingDateTimes = summary.rows.map((row) => row.filing_datetime).filter(Boolean).sort();
        const next = {
          ...summary,
          ...
          first_filing_datetime: filingDateTimes[0] || null,
          last_filing_datetime: filingDateTimes.at(-1) || null,
...
async function loadSecBuys(universe, options) {
  ...
  const cached = insider.buysByTickerFromCache(options.cache || {}, universe.map((row) => row.ticker));
  if (fromCacheOnly) {
    return {
...
function buildClusterSignal({ ticker, cluster, feature, meta, freshnessLimit, today, bootstrapSignals }) {
  ...
  return {
    ...
    meta: {
      ...
      first_filing_datetime: cluster.first_filing_datetime || null,
      last_filing_datetime: cluster.last_filing_datetime || null,
      has_10b5_1_plan: Boolean(cluster.has_10b5_1_plan),
      ...technicalMeta(feature),
    },
  };
}
...
function buildStandaloneSeniorSignal({ ticker, summary, feature, meta, freshnessLimit, today, bootstrapSignals }) {
  ...
  return {
    ...
    meta: {
      ...
      first_filing_datetime: summary.first_filing_datetime || null,
      last_filing_datetime: summary.last_filing_datetime || null,
...
async function run(options = {}) {
  ...
  const universeLoader = options.universeLoader || loadUniverse;
  const universeResult = await universeLoader();
  const { universe, source } = universeResult;
  ...
  return {
    ...
    diagnostics: {
      ...
      sp500_count: universeResult.sp500_count || null,
      nasdaq100_count: universeResult.nasdaq100_count || null,
      nyse_count: universeResult.nyse_count || null,
      universe_overlap_count: universeResult.overlap_count || null,
    ...
    extra: {
      rules: {
        ...
        note: "Combina compras SEC EDGAR, compras Dataroma y flujo semanal Dataroma compra/venta; filtra al universo configurado de la estrategia.",
      },
    },
  };
}
