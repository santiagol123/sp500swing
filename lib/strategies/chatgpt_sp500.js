...
const FALLING_KNIFE_WEEKLY_DROP_PCT = pctConfig("CHATGPT_SP500_FALLING_KNIFE_WEEKLY_DROP_PCT", 0.05);
const FALLING_KNIFE_MONTHLY_DROP_PCT = pctConfig("CHATGPT_SP500_FALLING_KNIFE_MONTHLY_DROP_PCT", 0.10);
const FALLING_KNIFE_RSI = Number(process.env.CHATGPT_SP500_FALLING_KNIFE_RSI || 25);
...
function technicalRiskReview(signal, { profile = DEFAULT_PROFILE, score = 0 } = {}) {
  ...
  const gap = technicalValue(signal, "entry_gap_1d");
  const ret1w = technicalValue(signal, "ret_1w");
  const ret1m = technicalValue(signal, "ret_1m");
  const belowSma20 = signal.meta?.above_sma20 === false;
  const belowSma50 = signal.meta?.above_sma50 === false;
  ...
  const fallingKnife =
    belowSma20 &&
    belowSma50 &&
    ((Number.isFinite(ret1w) && ret1w <= -FALLING_KNIFE_WEEKLY_DROP_PCT) ||
      (Number.isFinite(ret1m) && ret1m <= -FALLING_KNIFE_MONTHLY_DROP_PCT) ||
      (Number.isFinite(rsi) && rsi < FALLING_KNIFE_RSI));
  ...
  if (fallingKnife) {
    const details = [
      Number.isFinite(ret1w) ? `1s ${(ret1w * 100).toFixed(1)}%` : null,
      Number.isFinite(ret1m) ? `1m ${(ret1m * 100).toFixed(1)}%` : null,
      Number.isFinite(rsi) ? `RSI ${rsi.toFixed(1)}` : null,
    ].filter(Boolean);
    reasons.push(`caida acelerada bajo SMA20/SMA50 (${details.join(", ")}); esperar confirmacion`);
  }
...
function retagSignal(signal, { today, freshnessLimit, profile = DEFAULT_PROFILE }) {
  ...
  return {
    ...
    action: accepted ? "COMPRAR_LIMITADA" : technicalReview.falling_knife ? "ESPERAR_CONFIRMACION_TECNICA" : "ESPERAR_FILTRO_CONVICCION",
    reason: accepted
    ...
    plan: accepted
      ? `Entrada tras publicacion del Form 4/Dataroma; cerrar por tiempo a ${TARGET_HOLDING_SESSIONS} dias si no toca stop/TP antes.`
    ...
    meta: {
      ...
      falling_knife: technicalReview.falling_knife,
...
async function run(options = {}) {
  ...
  const fallingKnifeSignals = signals.filter((signal) => signal.meta?.falling_knife);
  ...
  return {
    ...
    diagnostics: {
      ...
      falling_knife_wait_count: fallingKnifeSignals.length,
      falling_knife_wait_tickers: fallingKnifeSignals.map((signal) => signal.ticker),
    },
    extra: {
      ...(base.extra || {}),
      rules: {
        ...
        falling_knife_weekly_drop_pct: FALLING_KNIFE_WEEKLY_DROP_PCT,
        falling_knife_monthly_drop_pct: FALLING_KNIFE_MONTHLY_DROP_PCT,
        falling_knife_rsi: FALLING_KNIFE_RSI,
        ...
        universe_label: profile.convictionLabel,
        note:
          `Automatiza el chat: compras Form 4 codigo P/Dataroma como evento de 1-5 dias, priorizando CEO/CFO/officers/directors, clusters, importe material y filings recientes.`,
      },
    },
  };
}
...
module.exports = {
  ...
  label: "ChatGPT SP500",
  description:
    "Compras fuertes de insiders del S&P 500: Form 4 codigo P/Dataroma, clusters, seniority, importe material y salida temporal a 5 dias.",
