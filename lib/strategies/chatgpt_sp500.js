...
const MAX_NEW_BUYS_PER_DAY = Number(process.env.CHATGPT_SP500_MAX_NEW_BUYS_PER_DAY || 3);
const MAX_BUYS_PER_SECTOR_PER_DAY = Number(process.env.CHATGPT_SP500_MAX_BUYS_PER_SECTOR_PER_DAY || 2);
...
function seniorityTitle(title = "") {
  ...
  if (/\b(ceo|chief executive|cfo|chief financial|coo|chief operating)\b/.test(text)) return "ceo_cfo";
...
function rejectionReasons(signal, today, freshnessLimit) {
  ...
  if (insiderCount < MIN_CONVICTION_INSIDERS) reasons.push(`menos de ${MIN_CONVICTION_INSIDERS} insiders`);
...
function retagSignal(signal, { today, freshnessLimit }) {
  const filingDate = lastFilingDate(signal);
  ...
  return {
    ...
    reason: accepted
      ? `Conviccion insider S&P 500: ${insiderCount} insiders senior/materiales compraron ${Math.round(valueUsd).toLocaleString("en-US")} USD; filing ${filingDate}${filingAge == null ? "" : ` (${filingAge} dias)`}.`
      : `${signal.reason} | filtro ChatGPT SP500: ${reasons.join("; ")}`,
    plan: accepted
    ...
    meta: {
      ...
      benchmark: "SPY",
      rule_source: "chat_pasted_text_insider_conviction",
    },
  };
}
...
async function run(options = {}) {
  ...
  const mapped = (base.signals || [])
    .map((signal) => retagSignal(signal, { today, freshnessLimit }))
  ...
  return {
    ...
    diagnostics: {
      ...
      execution_profile: "chatgpt_sp500_insider_conviction",
