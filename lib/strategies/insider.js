...
function findCluster(buys) {
  ...
  for (const anchor of sorted) {
    ...
    const filingDateTimes = members.map((m) => m.filing_datetime).filter(Boolean).sort();
    ...
    return {
      ...
      last_filing_datetime: filingDateTimes[filingDateTimes.length - 1] || null,
      ...
      insiders: insiders.map((m) => ({
        ...
        filing_datetime: m.filing_datetime || null,
...
function normalizeAllowedTickers(universeTickers = null) {
  if (!universeTickers) return null;
  return new Set([...universeTickers].map((ticker) => lookupTickerKey(ticker)));
}

function lookupTickerKey(ticker) {
  return String(ticker || "").trim().toUpperCase().replace(/\./g, "-");
}

function buysByTickerFromCache(cache = {}, universeTickers = null) {
  const allowed = normalizeAllowedTickers(universeTickers);
  ...
  for (const rows of Object.values(cache || {})) {
    ...
    for (const row of rows) {
      const ticker = lookupTickerKey(row.symbol);
      if (!ticker) continue;
...
function withFilingMetadata(rows, filing) {
  if (!Array.isArray(rows)) return [];
  return rows.map((row) => ({
    ...row,
    filing_date: row.filing_date || filing.filingDate || null,
    filing_datetime: row.filing_datetime || filing.filingAcceptedAt || filing.acceptanceDateTime || null,
    filing_acceptance_datetime: row.filing_acceptance_datetime || filing.acceptanceDateTime || null,
  }));
}
  ...
  for (const entry of rows) {
    ...
    for (const filing of filings) {
      ...
      if (Object.prototype.hasOwnProperty.call(cache, filing.accession)) {
        const cached = withFilingMetadata(cache[filing.accession], filing);
...
async function run(options = {}) {
  ...
  const cached = buysByTickerFromCache(options.cache || {}, universe.map((row) => row.ticker));
