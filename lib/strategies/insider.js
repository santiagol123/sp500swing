...
async function run(options = {}) {
  ...
  const { buysByTicker, cache, stats } = await collectInsiderBuys(universe, options);
  ...
  for (const { ticker, cluster } of clusters) {
    ...
    const fresh = staleDays <= SIGNAL_FRESH_DAYS;
    ...
    signals.push({
      ...
      action: fresh ? "COMPRAR_LIMITADA" : "SENAL_CADUCADA",
      reason: fresh
        ? `${cluster.insider_count} insiders compraron ${Math.round(cluster.total_value_usd).toLocaleString("en-US")} USD entre ${cluster.first_buy} y ${cluster.last_buy}`
        : `Cluster de ${cluster.last_buy}, demasiado antiguo (${Math.round(staleDays)} dias > ${SIGNAL_FRESH_DAYS})`,
  ...
  return {
    ...
    diagnostics: {
      ...
      signals_fresh: signals.filter((s) => s.action === "COMPRAR_LIMITADA" || s.authorized).length,
