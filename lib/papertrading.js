...
function profitLockConfig(workspace) {
  const cfg = workspace?.portfolio?.profit_lock;
  if (!cfg) return null;

  const firstTriggerPct = Number(cfg.first_trigger_pct);
  const breakevenLockPct = Number(cfg.breakeven_lock_pct);
  const secondTriggerPct = Number(cfg.second_trigger_pct);
  const secondLockPct = Number(cfg.second_lock_pct);

  if (
    !Number.isFinite(firstTriggerPct) ||
    !Number.isFinite(breakevenLockPct) ||
    !Number.isFinite(secondTriggerPct) ||
    !Number.isFinite(secondLockPct) ||
    firstTriggerPct <= 0 ||
    secondTriggerPct <= firstTriggerPct
  ) {
    return null;
  }

  return {
    first_trigger_pct: firstTriggerPct,
    breakeven_lock_pct: breakevenLockPct,
    second_trigger_pct: secondTriggerPct,
    second_lock_pct: secondLockPct,
  };
}
...
function profitLockStop(position, row, workspace) {
  const cfg = profitLockConfig(workspace);
  if (!cfg || !Number.isFinite(position.entry_price) || position.entry_price <= 0) return null;

  const high = Number.isFinite(row?.high) ? row.high : closeOf(row);
  if (!Number.isFinite(high)) return null;

  let stage = null;
  let nextStop = null;
  if (high >= position.entry_price * (1 + cfg.second_trigger_pct)) {
    stage = "LOCK_1_5";
    nextStop = position.entry_price * (1 + cfg.second_lock_pct);
  } else if (high >= position.entry_price * (1 + cfg.first_trigger_pct)) {
    stage = "BREAKEVEN";
    nextStop = position.entry_price * (1 + cfg.breakeven_lock_pct);
  }

  const currentStop = Number(position.stop_price);
  if (!stage || !Number.isFinite(nextStop) || (Number.isFinite(currentStop) && nextStop <= currentStop)) return null;

  return {
    stage,
    stop_price: round(nextStop, 4),
    trigger_high: round(high, 4),
  };
}
...
function updateOpenPositions(state, chartsByTicker, workspace, marketDate) {
  ...
  for (const position of state.positions) {
    ...
    for (const row of newRows) {
      ...
      if (hitStop) {
        const stopReason = position.profit_lock_stage ? "STOP_PROFIT_LOCK" : position.max_loss_stop_applied ? "STOP_MAX_LOSS" : "STOP";
      ...
      const lock = profitLockStop(position, row, workspace);
      if (lock) {
        position.stop_price = lock.stop_price;
        position.profit_lock_stage = lock.stage;
        position.profit_lock_trigger_high = lock.trigger_high;
        position.signal_meta = {
          ...(position.signal_meta || {}),
          profit_lock_stage: lock.stage,
          profit_lock_stop_price: lock.stop_price,
          profit_lock_trigger_high: lock.trigger_high,
        };
      }
    }

    const lastRow = rowFor(chart, marketDate);
    const shouldCheckMarkedStop =
      lastRow?.date === marketDate &&
      ((position.entry_date === marketDate && position.last_marked_date === marketDate) || position.profit_lock_stage);

    if (!exit && maxLossPct(workspace) && shouldCheckMarkedStop) {
      const marked = closeOf(lastRow);
      if (Number.isFinite(marked) && marked <= position.stop_price) {
        exit = {
          date: lastRow.date,
          price: marked,
          reason: position.profit_lock_stage ? "STOP_PROFIT_LOCK" : position.max_loss_stop_applied ? "STOP_MAX_LOSS" : "STOP",
        };
      }
    }
...
function marketTimeParts(value) {
...
function stampSignalDetection(signals = [], previousSignals = [], detectedAt = new Date().toISOString()) {
  const previousByTicker = new Map((previousSignals || []).map((signal) => [signal.ticker, signal]));

  for (const signal of signals || []) {
    const eventDate = insiderEventDateFromSignal(signal);
    const previous = previousByTicker.get(signal.ticker);
    const previousEventDate = insiderEventDateFromSignal(previous);
    const sameEvent = Boolean(eventDate && previousEventDate && eventDate === previousEventDate);
    const firstDetectedAt = sameEvent ? previous?.meta?.first_detected_at || detectedAt : detectedAt;
...
function signalTimingBlock(signal, marketDate, workspace) {
  if (!workspace?.portfolio?.require_new_insider_event_after_exit) return null;

  const eventDate = insiderEventDateFromSignal(signal);
  const detected = marketTimeParts(signal.meta?.first_detected_at);
  const eventInFuture = eventDate && eventDate > marketDate;
  const detectedAfterMarketDate = detected?.date && detected.date > marketDate;
  const detectedAfterClose = detected?.date === marketDate && detected.hour >= 16;
  if (!eventInFuture && !detectedAfterMarketDate && !detectedAfterClose) return null;

  let reason = `publicacion insider ${eventDate || "sin fecha"} posterior al mercado ${marketDate}`;
  if (!eventInFuture && detectedAfterMarketDate) {
    reason = `senal detectada el ${detected.date}, posterior al mercado ${marketDate}`;
  } else if (!eventInFuture && detectedAfterClose) {
    reason = `senal detectada tras el cierre de ${marketDate}`;
  }

  return {
    ticker: signal.ticker,
    action: "ESPERAR_PRIMER_PRECIO_EJECUTABLE",
    market_date: marketDate,
    signal_event_date: eventDate,
    first_detected_at: signal.meta?.first_detected_at || null,
    reason,
  };
}

function blockSignalTiming(signal, block) {
  signal.authorized = false;
  signal.action = block.action;
  signal.size_pct = 0;
  signal.reason = `${signal.reason} | entrada aplazada: ${block.reason}`;
  signal.meta = {
    ...(signal.meta || {}),
    timing_blocked: true,
    timing_block_reason: block.reason,
    timing_block_market_date: block.market_date,
  };
}
...
function isStopExit(reason) {
  return String(reason || "").includes("STOP");
}

function insiderNames(meta = {}) {
  return new Set(
    (meta.insiders || [])
...
function hasNewDistinctInsider(signal, lastTrade) {
  const previousNames = insiderNames(lastTrade?.signal_meta || {});
  const currentNames = insiderNames(signal?.meta || {});
  if (!previousNames.size || !currentNames.size) return null;
  return [...currentNames].some((name) => !previousNames.has(name));
}
...
function stopReentryCooldownBlock(signal, state, workspace, chart, marketDate) {
  const cooldownDays = Number(workspace?.portfolio?.stop_reentry_cooldown_days || 0);
  if (!Number.isFinite(cooldownDays) || cooldownDays <= 0) return null;

  const lastTrade = latestTradeForTicker(state.trades || [], signal.ticker);
  if (!lastTrade?.exit_date || !isStopExit(lastTrade.exit_reason)) return null;
  if (Number(lastTrade.pnl || 0) >= 0) return null;

  const rowsSinceStop = (chart?.rows || []).filter((row) => row.date > lastTrade.exit_date && row.date <= marketDate);
  const fallbackDays = dayDiff(marketDate, lastTrade.exit_date);
  const sessionsSinceStop = rowsSinceStop.length || fallbackDays;

  const currentRow = rowFor(chart, marketDate);
  const currentClose = closeOf(currentRow);
  const previousRows = (chart?.rows || []).filter((row) => row.date > lastTrade.exit_date && row.date < marketDate).slice(-3);
  const recentHigh = previousRows.length ? Math.max(...previousRows.map((row) => (Number.isFinite(row.high) ? row.high : closeOf(row)))) : null;
  const recoveredEntry = Number.isFinite(currentClose) && Number.isFinite(lastTrade.entry_price) && currentClose > lastTrade.entry_price;
  const brokeRecentHigh = previousRows.length >= 3 && Number.isFinite(currentClose) && Number.isFinite(recentHigh) && currentClose > recentHigh;
  const aboveSma20 = signal.meta?.above_sma20 === true;
  const technicalRecovery = recoveredEntry && aboveSma20;
  const newDistinctInsider = hasNewDistinctInsider(signal, lastTrade);
  const meaningfulStopLoss = lastTrade.exit_reason !== "STOP_PROFIT_LOCK" || Number(lastTrade.pnl_pct || 0) <= -0.005;

  if (newDistinctInsider === true || technicalRecovery) return null;
  if (sessionsSinceStop > cooldownDays && (!meaningfulStopLoss || newDistinctInsider == null)) return null;

  const confirmation = meaningfulStopLoss && newDistinctInsider === false
    ? "exigir insider distinto o cierre sobre entrada anterior y SMA20"
    : `esperar ${cooldownDays} sesiones o cierre sobre entrada anterior y SMA20`;
  ...
  return {
    ...
    action: "ESPERAR_CONFIRMACION_REENTRADA",
    reason: `stop reciente (${lastTrade.exit_date}); ${confirmation}${brokeRecentHigh ? "; maximo de 3 sesiones recuperado, pero tendencia aun debil" : ""}`,
  };
}

function blockSignalReentry(signal, block) {
  signal.authorized = false;
  signal.action = block.action || "ESPERAR_NUEVO_INSIDER";
  signal.size_pct = 0;
...
function pruneInvalidTimingPositions(state, workspace) {
  if (!workspace?.portfolio?.require_new_insider_event_after_exit) return [];

  const kept = [];
  const pruned = [];
  for (const position of state.positions) {
    const eventDate = position.signal_event_date || insiderEventDateFromMeta(position.signal_meta || {});
    if (!eventDate || !position.entry_date || eventDate <= position.entry_date) {
      kept.push(position);
      continue;
    }
    ...
    pruned.push({
      ...
      signal_event_date: eventDate,
      refunded_cost_basis: round(position.cost_basis, 2),
      reason: `entrada ${position.entry_date} anterior a publicacion ${eventDate}`,
    });
  }
...
function openNewPositions(state, signals, chartsByTicker, workspace, marketDate, blockedReentries = [], blockedTiming = []) {
  const opened = [];
  ...
  for (const signal of candidates) {
    ...
    const timingBlock = signalTimingBlock(signal, marketDate, workspace);
    if (timingBlock) {
      blockSignalTiming(signal, timingBlock);
      blockedTiming.push(timingBlock);
      continue;
    }
    ...
    const stopCooldownBlock = stopReentryCooldownBlock(signal, state, workspace, chart, marketDate);
    if (stopCooldownBlock) {
      blockSignalReentry(signal, stopCooldownBlock);
      blockedReentries.push(stopCooldownBlock);
      continue;
    }
...
function trackDay(state, { signals, charts, marketDate, workspace }) {
  ...
  const prunedTiming = pruneInvalidTimingPositions(state, workspace);
  ...
  const blockedTiming = [];
  const opened = openNewPositions(state, signals || [], chartsByTicker, workspace, date, blockedReentries, blockedTiming);
  ...
  return {
    ...
    blocked_timing: blockedTiming,
    pruned_reentries: prunedReentries,
    pruned_timing: prunedTiming,
