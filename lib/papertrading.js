...
function stopReentryCooldownBlock(signal, state, workspace, chart, marketDate) {
  ...
  if (sessionsSinceStop > cooldownDays) return null;
  ...
  if (recoveredEntry || brokeRecentHigh) return null;
  ...
  return {
    ...
    reason: `stop reciente (${lastTrade.exit_date}); esperar ${cooldownDays} sesiones o cierre sobre entrada anterior / maximo 3 sesiones`,
  };
}
