...
const DEFAULT_AUTO_RISK_BUDGET = 65;

function autoRiskBudget() {
  return finite(portfolioData.auto_risk_budget, DEFAULT_AUTO_RISK_BUDGET);
}
...
function sharesForAutoPosition(entry, stop) {
  const ticket = finite(portfolioData.ticket_size, 2000);
  const ticketShares = entry > 0 ? Math.floor(ticket / entry) : 0;
  const riskPerShare = Math.max(0, entry - stop);
  const riskShares = riskPerShare > 0 ? Math.floor(autoRiskBudget() / riskPerShare) : ticketShares;
  if (!ticketShares) return 0;
  return Math.max(1, Math.min(ticketShares, riskShares || ticketShares));
}

function tradeKey(trade) {
  return `${trade.ticker}:${trade.entry_date}`;
}
...
function enrichOpenPosition(position, prices, asOfDate) {
  ...
  return {
    ...
    auto_opened: Boolean(position.auto_opened),
    note: position.note || "",
  };
}
...
function recommendationToPosition(row, asOfDate) {
  const entry = finite(row.entry_zone_high, finite(row.last_close, 0));
  const stop = finite(row.invalid_below_price, 0);
  return {
    ticker: row.ticker,
    name: row.name || row.ticker,
    sector: row.gics_sector || "Sin sector",
    strategy: row.strategy_family || row.Accion || "SCANNER",
    entry_date: row.auto_entry_date || row.entry_date || asOfDate,
    entry,
    stop,
    target: finite(row.target_price, 0),
    shares: sharesForAutoPosition(entry, stop),
    max_sessions: 15,
    auto_opened: true,
    note: `Compra automatica desde senal ${row.Accion_Ejecucion}; limite usado ${round(entry, 4)}.`,
  };
}

function movementBuy(position, source = "manual") {
  return {
    date: position.entry_date,
    type: source === "auto" ? "BUY_AUTO" : "BUY_MANUAL",
    ticker: position.ticker,
    name: position.name || position.ticker,
    sector: position.sector || "Sin sector",
    price: round(position.entry, 4),
    shares: sharesFor(position),
    strategy: position.strategy || "SCANNER",
    note: position.note || "",
  };
...
function movementClose(trade) {
  return {
    date: trade.exit_date,
    type: trade.auto_closed ? `SELL_AUTO_${trade.result}` : `SELL_${trade.result || "CLOSE"}`,
    ticker: trade.ticker,
    name: trade.name || trade.ticker,
    price: round(trade.exit, 4),
    shares: trade.shares || sharesFor(trade),
    strategy: trade.strategy || "SCANNER",
...
function buildPortfolioSnapshot(rawRows = [], asOfDate = portfolioData.as_of, autoBuyRows = []) {
  ...
  for (const position of portfolioData.open_positions || []) {
    ...
    movements.push(movementBuy(position));
    if (trigger) {
      const closed = autoClosePosition(position, trigger);
      autoClosedTrades.push(closed);
      movements.push(movementClose(closed));
    } else {
      openPositions.push(enrichOpenPosition(position, prices, asOf));
    }
  }

  const closedTrades = (portfolioData.closed_trades || []).map(enrichClosedTrade).concat(autoClosedTrades);
  for (const trade of closedTrades) {
    if (!trade.auto_closed) {
      movements.push(movementBuy(trade));
      movements.push(movementClose(trade));
    }
  }

  const openKeys = new Set(openPositions.map(tradeKey));
  const openTickers = new Set(openPositions.map((row) => row.ticker));
  const closedTodayTickers = new Set(closedTrades.filter((row) => row.exit_date === asOf).map((row) => row.ticker));
  const autoOpened = [];

  for (const row of autoBuyRows || []) {
    if (!row?.portfolio_allowed) continue;
    if (openTickers.has(row.ticker) || closedTodayTickers.has(row.ticker)) continue;
    const position = recommendationToPosition(row, asOf);
    const key = tradeKey(position);
    if (openKeys.has(key)) continue;
    openKeys.add(key);
    openTickers.add(position.ticker);
    movements.push(movementBuy(position, "auto"));
    const quote = prices.get(position.ticker) || {};
    const trigger = findExitTrigger(position, quote, asOf);
    if (trigger) {
      const closed = autoClosePosition(position, trigger);
      ...
      autoOpened.push(position);
      openPositions.push(enrichOpenPosition(position, prices, asOf));
    }
  }

  movements.sort((a, b) => `${a.date}:${a.ticker}:${a.type}`.localeCompare(`${b.date}:${b.ticker}:${b.type}`));
  ...
  return {
    ...
    automation: {
      ...
      auto_opened_count: autoOpened.length,
      auto_risk_budget_per_trade: autoRiskBudget(),
      note: "La API deriva compras, cierres por stop, take profit o tiempo maximo usando Yahoo; no escribe en disco en Vercel.",
    },
...
function scannerContextFromPortfolio(snapshot) {
  ...
  const openTickers = new Set(openPositions.map((row) => row.ticker));
  const openSectorCounts = {};
  const openStrategyCounts = {};
  for (const row of openPositions) {
    const sector = row.sector || "SIN_SECTOR";
    const strategy = row.strategy || "SCANNER";
    openSectorCounts[sector] = (openSectorCounts[sector] || 0) + 1;
    openStrategyCounts[strategy] = (openStrategyCounts[strategy] || 0) + 1;
  }
  const buysToday = (snapshot.movements || []).filter((row) => row.date === snapshot.as_of && row.type.startsWith("BUY"));
