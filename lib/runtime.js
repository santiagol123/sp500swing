...
async function getSignals(workspace, options = {}) {
  ...
  if (!snapshot) {
    return {
      ...
      extra: {},
      warning:
        "Todavia no hay snapshot. El escaneo de insiders lo genera el GitHub Action (no cabe en una funcion de 60s por el limite de 10 peticiones/segundo de la SEC). Ejecuta `npm run track` para generarlo.",
    };
  }
