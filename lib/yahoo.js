...
// Devuelve filas {date, open, high, low, close, close_raw, volume}.
//
// `close` es el cierre AJUSTADO (lo que usan los indicadores) y open/high/low
// son los precios REALES sin ajustar. Esa es la semantica que espera
// lib/portfolio.js: la cartera real guarda precios de compra reales, asi que
// sus stops hay que compararlos contra maximos y minimos reales.
//
// Para el simulador se anade `close_raw`, de modo que el paper trading pueda
// trabajar entero en precios sin ajustar y no mezclar dos bases distintas.
async function fetchChart(symbol, { range = "1y", timeoutMs = 15000 } = {}) {
  ...
  for (let i = 0; i < timestamps.length; i += 1) {
    ...
    rows.push({
      ...
      close,
      close_raw: Number.isFinite(rawClose) ? rawClose : close,
      volume,
    });
  }
